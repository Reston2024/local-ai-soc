...
class TestCopilotSystemPrompt:
    ...
    def test_copilot_prompt_mentions_mitre(self):
        ...
        assert "mitre" in SYSTEM.lower() or "att&ck" in SYSTEM.lower()

    def test_template_sha256_is_populated(self):
        """TEMPLATE_SHA256 must be a non-empty 64-char hex string (SHA-256)."""
        from prompts.copilot import TEMPLATE_SHA256
    ...
    def test_template_name_is_set(self):
        """TEMPLATE_NAME must equal 'copilot'."""
        from prompts.copilot import TEMPLATE_NAME

        assert TEMPLATE_NAME == "copilot"

    def test_chat_module_re_exports_copilot_system(self):
        """backend.api.chat must still export _COPILOT_SYSTEM for backward compat."""
        from backend.api.chat import _COPILOT_SYSTEM
        from prompts.copilot import SYSTEM

        assert _COPILOT_SYSTEM is SYSTEM
