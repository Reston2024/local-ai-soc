...
must_haves:
  ...
  key_links:
    ...
    - from: "backend/api/explain.py"
      to: "backend/services/ollama_client.py"
      via: "request.app.state.ollama — confirmed pattern from research (NOT get_ollama_client() which is unverified)"
      pattern: "request.app.state.ollama.generate(...)"
---
...
<!-- OllamaClient access pattern — VERIFIED from research/code-examples section: -->
<!--   request.app.state.ollama.generate(...)                                     -->
<!--   Do NOT use get_ollama_client() from deps.py — that function's existence is  -->
<!--   UNVERIFIED and will cause ImportError on router mount if missing.            -->
...
IMPORTANT: Access OllamaClient via `request.app.state.ollama` — this is the pattern confirmed to exist
in the codebase (research audit, code examples section). Do NOT use `get_ollama_client()` from
deps.py — that function's existence is unverified and would cause an ImportError on router mount.
...
- OllamaClient accessed via request.app.state.ollama (not unverified get_ollama_client())
</success_criteria>
