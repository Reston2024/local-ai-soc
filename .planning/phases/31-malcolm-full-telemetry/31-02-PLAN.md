...
<task type="auto">
  ...
  <action>
    ...
    **Add NormalizationWriter to ubuntu/normalization_server.py (append before `if __name__ == "__main__":`):**
    ...
    Then append the NormalizationWriter class and lifespan wiring after the last route handler:
...
"
    ...
    Update the `app = FastAPI(...)` line to wire in the lifespan:
    ```python
    app = FastAPI(title="SOC Ubuntu Normalization Server", version="1.0.0", lifespan=lifespan)
    ```

    Note: The `lifespan` function must be defined BEFORE `app = FastAPI(...)` in the file. Reorder
    accordingly when writing the final file: imports → constants → ECS mappers → NormalizationWriter
    class → lifespan → app = FastAPI() → route handlers.
  ...
  <verify>
    <automated>uv run python -c "from ubuntu.normalization_server import app, NormalizationWriter; w = NormalizationWriter.__new__(NormalizationWriter); print('import OK', app.title); print('NormalizationWriter OK')"</automated>
  </verify>
  <done>
    normalization_server.py imports cleanly on desktop Python (FastAPI available).
    NormalizationWriter class is defined in normalization_server.py and calls _map_eve_doc() / _map_syslog_line().
    FastAPI app is created with lifespan=lifespan so the writer starts on uvicorn launch.
    ...
    Verify writer wiring: uv run python -c "from ubuntu.normalization_server import NormalizationWriter, app; assert app.router.lifespan_context is not None; print('Writer wired OK')"
  </done>
</task>
...
- NormalizationWriter class defined in normalization_server.py, calls _map_eve_doc() and _map_syslog_line()
- FastAPI app created with lifespan=lifespan so writer starts automatically on uvicorn launch
- On receipt of new raw EVE lines, $SOC_NORMALIZED_DIR/YYYY-MM-DD.ndjson.gz is populated (GET /normalized/latest returns 200, not 404)
- ubuntu/requirements.txt exists with fastapi + uvicorn + httpx
